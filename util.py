...
# def calc_loss(predict_heatmaps, label_map, temporal):
#     '''
...
#     target = label_map[:, 0, :, :, :]
#     initial_loss = tf.losses.mean_squared_error(target,predict)  # loss initial
#     total_loss = initial_loss

#     for t in range(temporal):
#         predict = predict_heatmaps[t+1]
#         target = label_map[:, t, :, :, :]
#         tmp_loss = tf.losses.mean_squared_error(target,predict)  # loss in each stage
#         total_loss += tmp_loss
#     return total_loss
...
def pred_images(heatmaps,step,temporal, save_dir):
    ...
    for t in range(temporal):                           # for each temporal
        ...
        _,pre = cv2.threshold(pre,0.3,1,cv2.THRESH_BINARY)
...
def pred_images2(heatmaps,label_map,step,temporal, save_dir):
    ...
    for t in range(temporal):                           # for each temporal
        ...
        for i in range(21):
            ...
            gth += label_map[b][t, :, :, i]
        # pre = pre/np.max(pre)
        _,pre = cv2.threshold(pre,0.01,1,cv2.THRESH_BINARY)
        # pre = scaler_model.fit_transform(pre)
        # _,gth = cv2.threshold(pre,.1,1,cv2.THRESH_BINARY)        
        pre = cv2.blur(pre,(5,5))
        output[0:45,  50 * t: 50 * t + 45] = pre
...
def output(heatmaps,temporal):
    ...
    scipy.misc.imsave(save_dir+'random_test_prediction.jpg', output)