...
class Net():
    ...
    def convnet1(self,img):
        ...
        with tf.name_scope('conv1'):
            x = pool2d(conv(img,128,9,act=tf.nn.relu))
            x = pool2d(conv(x,128,9,act=tf.nn.relu))
            x = pool2d(conv(x,128,9,act=tf.nn.relu))
            x = conv(x,32,5,act=tf.nn.relu)
            x = conv(x,512,9,act=tf.nn.relu)
            x = conv(x,512,1,act=tf.nn.relu)

            initial_heatmap = conv(x,self.outclass,1)
    ...
    def convnet3(self,hide_t):
        ...
        with tf.name_scope('conv3'):
            x = conv(hide_t,128,11,act=tf.nn.relu)  
            x = conv(x,128,11,act=tf.nn.relu) 
            x = conv(x,128,11,act=tf.nn.relu)
            x = conv(x,128,1,act=tf.nn.relu)
            x = conv(x,self.outclass,1)
            return x
    ...
    def lstm(self, heatmap, features, centermap, hide_t_1, cell_t_1):
        ...
        with tf.name_scope('LSTM'):
            ...
            gx = conv(xt,48,3)  # output: 45 * 45 * 48
            gh = conv(hide_t_1,48,3,bias=False) # output: 45 * 45 * 48
            g_sum = tf.add(gx, gh)
            gt = tf.nn.tanh(g_sum)

            ox = conv(xt,48,3)  # output: 45 * 45 * 48
            oh = conv(hide_t_1,48,3,bias=False) # output: 48 * 45 * 45
            o_sum = tf.add(ox, oh)
            ot = tf.nn.sigmoid(o_sum)

            ix = conv(xt,48,3)  # output: 45 * 45 * 48
            ih = conv(hide_t_1,48,3,bias=False)  # output: 48 * 45 * 45
            i_sum = tf.add(ix, ih)
            it = tf.nn.sigmoid(i_sum)

            fx = conv(xt,48,3)  # output: 48 * 45 * 45
            fh = conv(hide_t_1,48,3,bias=False)  # output: 48 * 45 * 45
            f_sum = tf.add(fx,fh)
...
if __name__ == '__main__':
    ...
    T=5
    images = tf.placeholder(tf.float32,shape=[None,368,368,T*3],name='temporal_images')
    ...
    c_map = tf.placeholder(tf.float32,shape=[None,368,368,1],name='gaussian_peak')
    net = Net(21,5) 
    maps = net.forward(images,c_map)
    with tf.Session() as sess:
        sess.run(tf.global_variables_initializer())
        sess.run(tf.local_variables_initializer())
        maps = sess.run(maps,feed_dict={images:image,c_map:cmap})
